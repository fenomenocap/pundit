...
export function HomeChat() {
  ...
  const stoppedDraftRef = useRef<StoppedDraft | null>(null);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (activeRequest) {
        ...
        if (activeRequest.autoAskKey && autoAskedRef.current === activeRequest.autoAskKey) {
  ...
  async function ask(
    ...
    const stoppedDraft = stoppedDraftRef.current;
    const reuseStoppedDraft = !chipFixtureContext && stoppedDraft?.prompt === trimmed;
    const requestTeamContext = chipFixtureContext
      ? undefined
      : reuseStoppedDraft
        ? stoppedDraft?.teamContext
        : teamContext;
    const requestFixtureContext = chipFixtureContext
      ?? (reuseStoppedDraft ? stoppedDraft?.fixtureContext : fixtureContext);
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => {
      ...
      void askRef.current(query, fixtureId ? { fixtureId } : undefined, autoAskKey);
    });
    return () => { cancelled = true; };
  }, [searchParams]);
        ...
        <div className="flex items-center gap-2">
          ...
          {loading ? (
            <Button
              ...
              size="sm"
              onClick={(event) => {
                // This button becomes Send after abort cleanup. Do not let the
                // same click submit the restored prompt as its default action.
