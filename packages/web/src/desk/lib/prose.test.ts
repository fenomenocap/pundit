...
describe("humaniseDeskCitationDates", () => {
  ...
  it("shortens a calendar date sitting in a citation parenthesis", () => {
    const out = humaniseDeskCitationDates(
      "Palmer is out ([Club update](https://example.com/news), 2026-09-11).",
    );
    assert.equal(
      out,
      "Palmer is out ([Club update](https://example.com/news) · 11 Sep).",
    );
  });
...
describe("formatDeskShortDate", () => {
  it("reads the UTC calendar day from an ISO instant", () => {
    ...
    assert.equal(formatDeskShortDate("2026-08-18"), "18 Aug");
