...
export const CLUBELO_LAST_KNOWN_BEFORE_KICKOFF_POLICY =
  "clubelo-last-known-window-on-or-before-utc-day-before-kickoff" as const;
...
export type ClubEloPreKickoffLookupPolicy =
  ...
  | typeof CLUBELO_LAST_KNOWN_BEFORE_KICKOFF_POLICY;
...
/**
 * Join fixtures to ClubElo windows. Policy date is the UTC day before
 * kickoff. Prefer a covering From/To window; otherwise use the last window
 * that ended before that day. Never use a later snapshot. Elo values are
 * ClubElo's own numbers — not interpolated.
 */
export function joinFixturesWithFromToElo(
  ...
  for (const fixture of fixtures) {
    ...
    const home = eloLastKnownOnOrBefore(byClub.get(homeName) ?? [], policyDate);
    const away = eloLastKnownOnOrBefore(byClub.get(awayName) ?? [], policyDate);
    ...
    rows.push({
      ...
      ratingProfile: profile,
      rankingDate: home.windowTo < policyDate || away.windowTo < policyDate
        ? (home.windowTo < away.windowTo ? home.windowTo : away.windowTo)
        : policyDate,
      homeElo: home.elo,
      awayElo: away.elo,
    });
...
export function validatePreKickoffCorpus(
  ...
  options: { failOnStaleDays?: boolean } = {}
  ...
  for (const row of rows) {
    ...
    if (deadline) {
      ...
      if (staleDays > CLUBELO_PRE_KICKOFF_MAX_STALE_DAYS && options.failOnStaleDays !== false) {
        errors.push(
