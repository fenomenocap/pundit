 ...
 * fail closed.
 */
function uniqueEloLookup(pool: readonly ClubEloValidityRow[]): ClubEloValidityLookup | null {
...
export function eloOnUtcDateFromValidityRows(
  ...
  return uniqueEloLookup(pool);
}

/**
 * Pre-match ClubElo: covering From/To window, else the last window that
 * ended before the policy date. Never uses From > policyDate (look-ahead).
 * Does not interpolate Elo values.
 */
export function eloLastKnownOnOrBefore(
  rows: readonly ClubEloValidityRow[],
  utcDate: string
): ClubEloValidityLookup | null {
  const covering = eloOnUtcDateFromValidityRows(rows, utcDate);
  if (covering) return covering;
  if (!DATE_RE.test(utcDate)) return null;
  const prior = rows.filter((row) => (
    row.to < utcDate
    && row.from <= row.to
    && observedOnOrBeforePolicy(row, utcDate)
  ));
  if (prior.length === 0) return null;
  const latestTo = prior.reduce((best, row) => (row.to > best ? row.to : best), prior[0].to);
  return uniqueEloLookup(prior.filter((row) => row.to === latestTo));
}
