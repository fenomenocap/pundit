...
describe("offline ClubElo per-club history", () => {
  ...
  it("uses the last ClubElo window before kickoff without looking ahead", () => {
    const rows = parseClubEloValidityCsv([
      "Rank,Club,Country,Level,Elo,From,To,date",
      "1,Arsenal,ENG,1,1975.82,2024-12-15,2024-12-31,2024-12-19",
      "1,Arsenal,ENG,1,1985.78,2025-01-05,2025-01-15,2025-01-10",
    ].join("\n"));
    expect(eloLastKnownOnOrBefore(rows, "2025-01-03")?.elo).toBe(1975.82);
    expect(eloLastKnownOnOrBefore(rows, "2025-01-03")?.windowTo).toBe("2024-12-31");
    expect(eloLastKnownOnOrBefore(rows, "2026-05-23")?.elo).toBe(1985.78);
    expect(eloLastKnownOnOrBefore(rows, "2024-12-14")).toBeNull();
    const laterOnly = parseClubEloValidityCsv([
      "Rank,Club,Country,Level,Elo,From,To,date",
      "1,Arsenal,ENG,1,1985.78,2025-01-05,2025-01-15,2025-01-10",
    ].join("\n"));
    expect(eloLastKnownOnOrBefore(laterOnly, "2025-01-03")).toBeNull();
  });
