...
export function validateResponseCorrectness(answer, citations, grounding, expectation = {}) {
  ...
  if (expectation.expectCorrectHighLineGeometry) {
    ...
    const pushedLineShrinksBehindSpace = /\b(?:back (?:four|line)|defen[cs](?:e|ive line))\b[^.!?\n]{0,50}\b(?:push|step|move)\w*\s+up\b[^.!?\n]{0,100}\b(?:space|gap|room)\s+behind\b[^.!?\n]{0,30}\b(?:shrink|reduce|compress|close|limit|narrow|minimi[sz]|decreas|shorten)\w*\b/i.test(text);
    const invertedTurnoverLocation = /\bmidfield turnovers?\b[^.!?\n]{0,80}\bcloser\b[^.!?\n]{0,30}\b(?:your|their|the team['’]s|its) own goal\b/i.test(text);
    const invertedMidfieldSpacing = [
      /(?:\b(?:deeper|closer)\b[^.!?\n]{0,40}\bmidfield\b[^.!?\n]{0,60}\b(?:in relation to|relative to|towards?|closer to)\b[^.!?\n]{0,24}\b(?:the )?(?:back|defensive) line\b|\bmidfield\b[^.!?\n]{0,30}\b(?:drops?|sits?|moves?)\b[^.!?\n]{0,24}\bcloser\b[^.!?\n]{0,20}\b(?:to|towards?)\b[^.!?\n]{0,20}\b(?:the )?(?:back|defensive) line\b)[^.!?\n]{0,80}(?:\b(?:more|increasingly|greater)\b[^.!?\n]{0,24}\bisolat(?:ed|ion)\b[^.!?\n]{0,24}\b(?:defenders?|defen[cs]e|back line)\b|\b(?:defenders?|defen[cs]e|back line)\b[^.!?\n]{0,24}\b(?:more|increasingly|greater)\b[^.!?\n]{0,24}\bisolat(?:ed|ion)\b)/i,
      /\b(?:defenders?|defen[cs]e|back line)\b[^.!?\n]{0,30}\b(?:more|increasingly|greater)\b[^.!?\n]{0,24}\bisolat(?:ed|ion)\b[^.!?\n]{0,40}\bcloser\b[^.!?\n]{0,24}\bmidfield\b[^.!?\n]{0,30}\b(?:drops?|sits?|moves?)\b[^.!?\n]{0,20}\b(?:to|towards?)\b[^.!?\n]{0,20}\b(?:the )?(?:back|defensive) line\b/i,
    ].some((pattern) => pattern.test(text));
    const backwardsHighLine = conceptualHighLineError || pushedLineShrinksBehindSpace || invertedTurnoverLocation || invertedMidfieldSpacing || [
