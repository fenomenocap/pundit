...
* Search calls are capped at 10 seconds. MiniMax's output budget is **8,192** tokens, including provider reasoning tokens
* Requires a MiniMax key on the API server (`MINIMAX_INFERENCE_API_KEY`, falling back to `MINIMAX_API_KEY`)
