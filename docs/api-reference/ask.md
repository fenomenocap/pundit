...
`presentation.responseMode` tells the client which settle was used (`pricing-desk`, `totals`, `btts`, `exact-score`, `player-or-scorer`, and so on). Follow-ups that name a priced grid market — 1X2, over/under 2.5 including `o2.5`/`U2.5`, BTTS, or a scoreline board — are composed from those facts without MiniMax. A market that is not on the grid is refused rather than answered with the 1X2 favourite.
