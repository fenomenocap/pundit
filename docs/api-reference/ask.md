...
Match grounding includes `pHome`, `pDraw`, `pAway`, `pOver2_5`, `pUnder2_5`, `pBttsYes`, `pBttsNo`, `topScores`, `homeFieldAdvantage`, nullable Stake columns, sparse Kalshi/Polymarket `oddsSources`, and a server-owned `pricing` object (fair odds `1/p`, `evPct` only when a captured decimal exists).

`presentation.responseMode` tells the client which settle was used (`pricing-desk`, `totals`, `btts`, `exact-score`, `player-or-scorer`, and so on). A homepage match briefing is `match-preview`; `Tactical matchup` / `Who decides it?` is `match-follow-up`. Neither is the pricing desk, and a current-news conflict cannot blank those takes. Follow-ups that name a priced grid market — 1X2, over/under 2.5 including `o2.5`/`U2.5`, BTTS, or a scoreline board — are composed from those facts without MiniMax. A market that is not on the grid is refused rather than answered with the 1X2 favourite.
...
* Evidence-required turns plan up to **6** searches. The first generation may continue for up to **2** further turns. Inference, retries, and searches share one budget of **10** provider calls
* Search calls are capped at 25 seconds. The answer model's output budget is **8,192** tokens, including provider reasoning tokens
* Requires `OPENROUTER_API_KEY` on the API server. `MINIMAX_API_KEY` answers only when that key is unset and does not serve search
