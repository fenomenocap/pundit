...
Grounded assistant messages show a match, recognized-fixture, competition, or season-outlook badge; general answers are labelled separately, while discovery-only candidates receive no fixture badge. A recognized fixture remains the context for match follow-ups, including a temporary table/season detour, until a new explicit recognized matchup replaces it. `current table` and `current standings` refer to the most recent competition you named; with none in view, Pundit uses the Premier League as its only league-style table. Other vague pronouns do not silently inherit a competition. Use **New Chat** to clear conversation and retained fixture context. Up to twelve history turns (six exchanges) carry forward for follow-ups. **Copy** and **Share** actions are available on assistant answers.
