...
**Today**, that's an analysis desk: a live fixture slate, an analyst pane on `POST /api/ask`, and a local paper lab (Paper / Draft / Vaults). The engine is a Poisson score matrix with a Dixon–Coles low-score correction. Expected goals are a fixed 2.70 total from a reviewed, pinned ClubElo strength artifact, plus home-field advantage. Around that sit ESPN standings, a complete-schedule-gated Premier League season simulator, and reference Stake/Kalshi/Polymarket prices — all read-only, informational, nothing to trade. A fitted attack/defence model is registered and not the champion. Production does not contact ClubElo at runtime. Credibility is shown openly through rolling club-season calibration and a frozen World Cup 2026 backtest.
