...
**Status:** Diagnosis preserved. Production `main` (`4784eef`, PR #186) has Phase 0, a sealing 90-minute ledger, Phase 1 calibrator (`productionAutoLoad: false`), a registered-not-activated fitted Dixon–Coles challenger, labelled Consensus, **and** the chronological evaluator. Do not remake Phase 0. Do not activate the challenger unless a human says **promote**.
...
**Shipped on `main` (`4784eef`, Railway API; Vercel web may lag when `packages/web` is unchanged):** Phase 0 fixed-total 2.70; leftovers (season-sim samples the DC grid; Fundamental version `"2"`; desk uses server Over 2.5); 90-minute ledger seal with 15-minute checkpoint; offline Phase 1b calibrator (`productionAutoLoad: false`); labelled Consensus that refits λ; fail-closed fitted Dixon–Coles challenger trained on **760/760** PL fixtures; chronological research evaluator from PR #186. Pre-kickoff ClubElo uses the last published From/To window on or before the UTC day before kickoff — ClubElo's own number, not an interpolation, and never a later snapshot. Artifact `dixon-coles-mle@4cfcbe57…`. Do **not** quote the September 14 rolling-origin Brier figures as holdout evidence. The 2026-09-15 review’s chronological evaluator refits each origin and still does **not** recommend promotion.

**Live ledger Phase 1 (updated 2026-09-15):** Railway `/api/evaluation/club-season` had 54 official `scheduled_window` rows (40 PL, 14 UCL quals) plus 8 excluded legacy post-kickoff seals. Fit on the earlier official Elos: 1.40 / 0 / −0.115. Shipped recomputed Brier **0.668** vs fitted **0.666** — not a clear win, PL n below a 40-holdout bar, bootstrap ΔBrier does not exclude 0. **Do not change shipped 1.35 / 42 / −0.1.** Re-run after more official PL seals. First Fundamental `"2"` seal is Leeds–Newcastle (`401879280`).
