...
The Schema-12 production evaluator fails certification when delivered answers contain internal jargon (`Dixon-Coles`, `ClubElo`, `model-grounded`), raw search/tool payloads, orphaned section labels, or a structurally incomplete ending. Match-grounded evaluation checks each explicit combined-scoreline percentage against the scoped grounding at the precision stated and rejects a home/away team claim whose named scorelines point the other way; the permanent scenario independently exercises the built settled sanitizer on a known-wrong aggregate. The high-line scenario rejects claims that a high line shrinks the space behind the defence or between the line and goalkeeper and requires the real space-behind trade-off. These are independent delivery checks, not substitutes for the API's deterministic guards. `validateTeamNewsDiscipline` additionally fails any answer asserting an injury, suspension, or lineup detail without naming a source and date — unless it explicitly states no verified update was established. Error-copy checks separately reject 400 bodies that leak schema field names.
